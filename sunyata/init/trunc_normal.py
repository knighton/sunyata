...
def _trunc_normal_stds(shape, mean=0, std=1, dtype='float32', min_stds=-2,
                       max_stds=2):
...
def _trunc_normal(shape, mean=0, std=1, dtype='float32', min_stds=None,
    ...
    return _trunc_normal_stds(shape, mean, std, dtype, min_stds, max_stds)


class TruncNormal(Initializer):
    ...
    def __call__(self, shape, dtype='float32', meaning=None):
        return _trunc_normal(shape, self.mean, self.std, dtype, self.min_stds,
