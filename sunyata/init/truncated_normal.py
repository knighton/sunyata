...
def _truncated_normal_stds(shape, mean=0, std=1, dtype='float32', min_stds=-2,
                           max_stds=2):
...
def _truncated_normal(shape, mean=0, std=1, dtype='float32', min_stds=None,
    ...
    return _truncated_normal_stds(shape, mean, std, dtype, min_stds, max_stds)
...
class TruncatedNormal(Initializer):
    ...
    def __call__(self, shape, dtype='float32', meaning=None):
        return _truncated_normal(shape, self.mean, self.std, dtype,
                                 self.min_stds, self.max_stds, self.min_value,
                                 self.max_value, self.default_min_stds,
                                 self.default_max_stds)
